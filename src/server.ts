...
disabled. To enable it, do `/post_session_msg enable: true`";
    ...
    ProcessForever(): void {
        ...
        // void Promise.all(this.queues.map(async queue => {
        //     while (true) {

        //         while (queue.update_time > new Date() && queue.update_time.getTime() !== 0);

        //         void this.getUpcomingHoursTable(queue.name).then(async res => {
        //             await queue.UpdateSchedule(res);
        //         });
        //     }
        // }))

        this.queues.map(queue => {
            const timeUntilNextUpdate: number = queue.update_time.getTime() - (new Date()).getTime();
            setInterval(() => {
                // update logic here
            }, timeUntilNextUpdate);
        }
